#0002change-dont-allow.cmd
#registerTester ignores lines which start with a # and blank lines
#the other interfaces also ignore blank lines

#the clerk id
100
#the clerk passcode
100

#this is plu 1 for 100.00
10000@2

#subt (in case we're running this manually)
/subt

#tender it to check: it shouldn't allow tendering as check as check doesn't allow change
20000/tender1

#so we have something to see, tender it as cash now
10000/tender0
